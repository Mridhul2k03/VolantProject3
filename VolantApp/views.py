...
from .decorators import login_required, login_required_admin
...
class RemoveOrderItem(View):
    def get(self,request,*args, **kwargs):
        id = kwargs.get('id')
        item = OrderModel.objects.get(id=id)
        item.delete()
        
        subject = f'Order Cancelled'
        message = f'Dear {item.user.username},\n\nYour order has been cancelled. \n\nThank you for choosing our shop.'
        from_email = settings.EMAIL_HOST_USER
        to_email = [item.email]
        send_mail(subject, message, from_email, to_email, fail_silently=False)
...
class UserOrderView(ListView):
 ...
 # _________________Admin views starts________________________
